...
if gh run watch "$RUN_ID" --exit-status; then
  osascript -e 'display notification "Deploy succeeded!" with title "Dineros" sound name "Glass"'
  echo "Deploy complete!"
else
  osascript -e 'display notification "Deploy FAILED" with title "Dineros" sound name "Basso"'
